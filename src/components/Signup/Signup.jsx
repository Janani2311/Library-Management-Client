...
const Signup = () => {   
    ...
    const handleSubmit = async(values) => {  
        ...
        try {  
            let res = await AxiosService.post(`${ApiRoutes.SIGNUP.path}`, values, { headers: { authenticate: ApiRoutes.SIGNUP.auth } });  
            if (res.status === 201) {  
                ...
                >  
                    {({ values }) => (  
                        <Form>  
                            ...
                            <ErrorMessage component='div' className={styles.errorMsg} name='firstName' />  

                            <label className={styles.label}>Last Name</label>  
                            <Field className={styles.field} id='lastName' name='lastName' />  
                            <ErrorMessage component='div' className={styles.errorMsg} name='lastName' />  

                            <label className={styles.label} htmlFor='email'>Email</label>  
                            <Field className={styles.field} id='email' name='email' />  
                            <ErrorMessage component='div' className={styles.errorMsg} name='email' />  
                            
                            <label className={styles.label} htmlFor='password'>Password</label>  
                            <Field className={styles.field} id='password' name='password' type="password" />  
                            <ErrorMessage component='div' className={styles.errorMsg} name='password' />  

                            <label className={styles.label} htmlFor='phone'>Phone Number</label>  
                            <Field className={styles.field} id='phone' name='phone' />  
                            <ErrorMessage component='div' className={styles.errorMsg} name='phone' />  
                            ...
                            <p className="text-center mt-4">  
                                Already Registered? <span className="font-medium text-blue-600 hover:underline"><Link to="/login">Sign In?</Link></span>  
                            </p>  
                        </Form>  
                    )}  
                </Formik>  
